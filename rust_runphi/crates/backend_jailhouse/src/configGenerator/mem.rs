...
fn get_options(c: &configGenerator::Backendconfig) -> MemOptions{
    MemOptions{
        rpu_req: c.rpu_req,
        no_ram_region: c.used_rcpus.is_empty() && c.cpus == 0,
        skip_ivshmem: c.net == "none",
        soft_core : !c.soft_core_mem.is_empty(),
    }
}
        ...
        if let Some(regions) = mem_regions.get("regions").and_then(|r| r.as_array()) {
            for region in regions {
                if let Some(template_name) = region.as_str() {
                    ...
                    if options.skip_ivshmem
                        && (template_name == "IVSHMEM_DEMO_TEMPLATE" || template_name == "IVSHMEM_TEMPLATE")
                    {
                        continue;
                    }
                    if !options.rpu_req
                        && (template_name == "TCMA_TEMPLATE" || template_name == "TCMB_TEMPLATE")
                    {
                        continue;
                    }
                    if options.no_ram_region
                        && (template_name == "RAM0_TEMPLATE" || template_name == "RAM_TEMPLATE")
                    {
                        continue;
                    }
                    if !options.soft_core
                        && (template_name == "RAM_SOFTCORE_TEMPLATE")
                    {
                        continue;
                    }
                    ...
                    if template_name == "RAM_SOFTCORE_TEMPLATE"{
                        let mem_params: Vec<&str> = soft_core_mem.split(';').map(|s| s.trim()).collect();
                        template = template.replace("{phys_start}", mem_params[0]);
                        template = template.replace("{virt_start}", mem_params[1]);
                        template = template.replace("{size}", mem_params[2]);
                    }
...
pub fn memconfig(
    ...
    let num_regions = match count_mem_regions(&file_path, &mem_options) { 
    Ok(count) => count,
    Err(e) => {
    ...
    let re_f = Regex::new(&pattern_fpga)?;
    let re_r = Regex::new(&pattern_rcpu)?;
    if let Some(pos) = re_f.find(&c.conf) { //insert after fpga if present
        c.conf
            .insert_str(pos.end(), &format!("\n{}\n", linetoinsert));
    } else if let Some(pos) = re_r.find(&c.conf) { // else insert after rcpu if present
        c.conf
        .insert_str(pos.end(), &format!("\n{}\n", linetoinsert));
    }
    else {
        return Err("__u64 rcpus; or __u64 fpga_regions; not found".into());
    }
...
fn count_mem_regions<P: AsRef<Path>>(file_path: P, mem_options: &MemOptions) -> Result<usize, Box<dyn std::error::Error>> {
    ...
    for region in mem_regions {
        if let Some(region_name) = region.as_str() {
            match region_name {
                "IVSHMEM_DEMO_TEMPLATE" => count = if mem_options.skip_ivshmem {count} else {count+5},
                "IVSHMEM_TEMPLATE" => count = if mem_options.skip_ivshmem {count} else {count+4},
                "TCMA_TEMPLATE" | "TCMB_TEMPLATE" =>  count = if mem_options.rpu_req {count+1} else {count},
                "RAM_TEMPLATE" | "RAM0_TEMPLATE" => count = if mem_options.no_ram_region {count} else {count+1},
                "RAM_SOFTCORE_TEMPLATE" => count = if mem_options.soft_core {count+1} else {count},
                _ => count += 1,
            }
        }
    }
